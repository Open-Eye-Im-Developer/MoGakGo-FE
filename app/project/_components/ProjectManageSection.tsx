...
function ProjectManageSection() {
  const { project } = useGetCurrentProjectQuery();

  return (
    <main className="flex h-full w-full items-center justify-center">
      {project && project.length === 0 && (
        <div className="flex h-[300px] flex-col items-center justify-between rounded-xl border-4 border-dotted p-10">
      ...
      {project && project.length > 0 && (
        <ProjectCardContainer project={project[0]} />
      )}
    </main>
  );
}
